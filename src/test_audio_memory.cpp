...
bool test_s24_hint_survives_clear() {
    ...
    ring.setS24PackModeHint(DirettaRingBuffer::S24PackMode::MsbAligned);
    ring.clear();   // pause -> resume

    // Samples 44 33 22 00: with the hint the ring takes bytes 1-3 (33 22 00);
    // if clear() had dropped it, sniffing sees b0 != 0 and b3 == 0 and picks
    // LSB (44 33 22) — so this assertion fails on the old clear().
    std::vector<uint8_t> in;
    fillS24Pattern(in, 64);
    for (size_t i = 3; i < in.size(); i += 4) in[i] = 0x00;
    ...
    TEST_ASSERT(out[0] == 0x33 && out[1] == 0x22 && out[2] == 0x00,
                "Hint (MSB) still applied after clear() — bytes 1-3, not sniffing's LSB pick");
